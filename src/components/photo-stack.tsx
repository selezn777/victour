...
// 650→450→300, затем ПО ОДНОЙ при 130 ("укачивает"), затем СТОПКАМИ по 2
// (280, 4 этапа при 8 фото) — Виктор: 4 отдельных этапа с паузой между
// ними создают "дисфокус". Вернулись к падению ПО ОДНОЙ, но сильно
// быстрее: "супер быстро, как будто колода выпадает / перелистывают
// страницы", весь каскад по ощущению — как 1–1.5 прежних "выпада"
// (не 1120мс как было у 4 этапов). Короткий интервал сам по себе не
// решает — см. DROP_ENTER_DURATION_MS ниже, без укорачивания посадки
// карточки много кадров одновременно "в полёте" и это снова дисфокус.
...
// Длительность самой посадки карточки (раньше 610мс, общее для входа и
// см. transitionDuration ниже) — при быстром DROP_STAGGER_MS длинная
// посадка держит сразу несколько карточек "в полёте" одновременно, это и
// есть дисфокус, а не сам интервал. Короче — каждая карточка долетает
// почти сразу за предыдущей, каскад читается как одно быстрое падение.
const DROP_ENTER_DURATION_MS = 260
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        ...
        // на неё следующая. По одной (не стопками) — короткий
        // DROP_STAGGER_MS + короткий DROP_ENTER_DURATION_MS вместе дают
        // единый быстрый каскад, а не отдельные этапы с паузой.
      ...
      {photos.map((src, i) => (
        <div
          ...
          style={{
            ...
            // DROP_ENTER_DURATION_MS — только сама посадка карточки, не
            // lifted (shuffle по тапу) и не DROP_STAGGER_MS/
            // DROP_INITIAL_DELAY_MS (те про паузу МЕЖДУ карточками, не про
            // длительность самого перехода).
