    ...
    <div className="flex h-full w-full flex-col overflow-hidden">
      ...
      <div className="relative h-[36%] shrink-0 sm:h-[min(40%,34svh)] lg:h-[min(40%,30svh)]">
        {stackImages && stackImages.length > 1 ? (
          // Все фото в стопке — реально 1:1 (проверено sips: 672x672,
          // 800x800, 900x900, 672x672, 756x756...) — Виктор специально
          // готовит их квадратными именно чтобы верстать без обрезки и без
          // выбора "какая часть кадра главная" (см. тот же принцип у
          // коллажа на IntroSlide, COLLAGE_PHOTOS). aspect-square на самом
          // контейнере (не только с sm) — раньше контейнер растягивался на
          // всю ширину слайда при небольшой высоте, и повёрнутые фото в
          // PhotoStack (см. STACK_TRANSFORM) торчали острыми краями за
          // пределы кадра, оставляя пустые клинья по бокам вместо
          // полноценного заполнения; попытка ужать до aspect-[6/5] всё
          // равно была не квадратом — Виктор: "куда делся квадрат, почему
          // фотки растягиваются в прямоугольник".
          <div className="mx-auto aspect-square h-full w-auto p-5 pb-8 sm:p-7 sm:pb-10">
        ...
        <ValuesSlide
          ...
          points={[
            ...
            },
            {
              ...
              body: "Ни одной обязательной лавки, где вожатому (гиду) капает процент с ваших покупок - заезжаем только туда, что интересно вам.",
            },
            {
