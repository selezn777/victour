...
export function SlideDeck({
  ...
  // var(--site-header-h) — реальная высота хедера, измеренная ResizeObserver'ом
  // в SiteHeader/TourHeader (см. use-header-height-var.ts) — раньше тут был
  // hardcoded rem (3.5rem/4rem под разные брейкпоинты), разошедшийся с
  // фактической высотой хедера (h-16/sm:h-20 + border) на 9-17px, из-за чего
  // низ последнего слайда (кнопка "Выбрать тур") уезжал за экран. Один var
  // работает на всех брейкпоинтах сразу. БЕЗ запятой-фолбэка внутри
  // квадратных скобок: var(--x,4rem) прямо в Tailwind arbitrary value тихо
  // ломал генерацию правила (класс не попадал в собранный CSS вообще) —
  // .swiper оставался без height, и Swiper насчитывал огромную битую высоту
  // (1.3M px на проде, сайт выглядел пустым). Дефолт --site-header-h — в
  // globals.css :root (первый пейнт до гидратации), запятая тут не нужна.
