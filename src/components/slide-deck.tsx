...
export function SlideDeck({
  ...
  // в SiteHeader (см. комментарий там) — раньше тут был hardcoded rem
  // (3.5rem/4rem под разные брейкпоинты), разошедшийся с фактической высотой
  // хедера (h-16/sm:h-20 + border) на 9-17px, из-за чего низ последнего
  // слайда (кнопка "Выбрать тур") уезжал за экран. Один var работает на всех
  // брейкпоинтах сразу — ResizeObserver сам обновляет его при смене высоты
  // хедера, отдельные sm:-варианты больше не нужны. 3.5rem — запасной
  // fallback на случай, если SiteHeader ещё не смонтировался/не в дереве.
  className = "h-[calc(100dvh-var(--site-header-h,3.5rem))] w-full",
  direction = "vertical",
