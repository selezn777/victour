...
export function BookingActions({
  ...
  const showStatusActions = status === "pending" || status === "alt_proposed"
  const showPaymentAction = paymentStatus === "unpaid" && status !== "cancelled"

  if (!showStatusActions && !showPaymentAction) return null

  return (
    <div className="flex flex-wrap items-center gap-2">
      {showPaymentAction && (
        <Button size="sm" variant="secondary" disabled={isPending} onClick={confirmPayment}>
          Предоплата получена
        </Button>
      )}
      {showStatusActions && (
